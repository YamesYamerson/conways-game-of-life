  ...
  return (
    <div className="w-full bg-gray-50 shadow-sm">
      <div className="max-w-7xl mx-auto py-3 px-4 flex flex-row items-center gap-6 justify-center">
        {Object.entries(grouped).map(([type, patternsInTypeRaw]) => {
          const patternsInType = patternsInTypeRaw ?? [];
          return (
            <div
              key={type}
                  ...
                  {patternsInType.map((pattern) => (
                    <div key={pattern.name} className="relative flex flex-col items-center">
                      <button
                        ...
                        className="px-3 py-1 min-w-[140px] text-left bg-indigo-100 text-indigo-700 rounded hover:bg-indigo-200 transition-colors text-sm font-medium shadow-sm"
                      >
                        {pattern.name}
                      </button>
